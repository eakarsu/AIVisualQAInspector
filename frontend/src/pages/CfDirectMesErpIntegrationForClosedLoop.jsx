...
export default function CfDirectMesErpIntegrationForClosedLoop() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-direct-mes-erp-integration-for-closed-loop-quality-control/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-direct-mes-erp-integration-for-closed-loop-quality-control', project: 'AIVisualQAInspector' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-direct-mes-erp-integration-for-closed-loop-quality-control/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Direct MES/ERP integration for closed-loop quality control</h1>
