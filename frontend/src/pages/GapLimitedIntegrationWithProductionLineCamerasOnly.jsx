...
export default function GapLimitedIntegrationWithProductionLineCamerasOnly() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-integration-with-production-line-cameras-only-generic-integrations/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-limited-integration-with-production-line-cameras-only-generic-integrations', project: 'AIVisualQAInspector' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-integration-with-production-line-cameras-only-generic-integrations/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Limited integration with production-line cameras (only generic integrations module)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AIVisualQAInspector</p>
      </div>
