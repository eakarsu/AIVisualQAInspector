...
// Feature: Root cause correlation tying defects to process parameters (temperature, speed, materials)
...
export default function CfRootCauseCorrelationTyingDefectsToProcess() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-root-cause-correlation-tying-defects-to-process-parameters/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-root-cause-correlation-tying-defects-to-process-parameters', project: 'AIVisualQAInspector' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-root-cause-correlation-tying-defects-to-process-parameters/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Root cause correlation tying defects to process parameters (temperature, speed, materials)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIVisualQAInspector</p>
      </div>
