...
export default function CfSupplierQualityTrackingScoringSupplierDefectContributions() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-supplier-quality-tracking-scoring-supplier-defect-contributions/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-supplier-quality-tracking-scoring-supplier-defect-contributions', project: 'AIVisualQAInspector' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-supplier-quality-tracking-scoring-supplier-defect-contributions/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Supplier quality tracking scoring supplier defect contributions</h1>
