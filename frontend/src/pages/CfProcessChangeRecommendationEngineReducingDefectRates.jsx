...
export default function CfProcessChangeRecommendationEngineReducingDefectRates() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-process-change-recommendation-engine-reducing-defect-rates/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-process-change-recommendation-engine-reducing-defect-rates', project: 'AIVisualQAInspector' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-process-change-recommendation-engine-reducing-defect-rates/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Process-change recommendation engine reducing defect rates</h1>
