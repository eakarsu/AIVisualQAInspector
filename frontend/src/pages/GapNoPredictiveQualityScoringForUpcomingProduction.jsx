...
export default function GapNoPredictiveQualityScoringForUpcomingProduction() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-predictive-quality-scoring-for-upcoming-production-runs/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-predictive-quality-scoring-for-upcoming-production-runs', project: 'AIVisualQAInspector' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-predictive-quality-scoring-for-upcoming-production-runs/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No predictive quality scoring for upcoming production runs</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AIVisualQAInspector</p>
      </div>
