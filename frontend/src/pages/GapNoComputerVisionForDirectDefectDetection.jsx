...
export default function GapNoComputerVisionForDirectDefectDetection() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-computer-vision-for-direct-defect-detection-from/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-computer-vision-for-direct-defect-detection-from', project: 'AIVisualQAInspector' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-computer-vision-for-direct-defect-detection-from/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No computer vision for direct defect detection from images</h1>
