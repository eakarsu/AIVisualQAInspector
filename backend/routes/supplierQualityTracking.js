...
// Supplier quality tracking scoring supplier defect contributions
// Feature: supplier-quality-tracking
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AIVisualQAInspector" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Supplier quality tracking scoring supplier defect contributions.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'supplier-quality-tracking', payload, result);
    res.json({ success: true, feature: 'supplier-quality-tracking', ...result });
  } catch (err) {
    console.error('supplier-quality-tracking analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AIVisualQAInspector". Return strict JSON only.',
      `Feature: Supplier quality tracking scoring supplier defect contributions. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'supplier-quality-tracking', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'supplier-quality-tracking', title: 'Supplier quality tracking scoring supplier defect contributions', project: 'AIVisualQAInspector' });
});
