...
// Process-change recommendation engine reducing defect rates
// Feature: process-change-recommender
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Process-change recommendation engine reducing defect rates.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'process-change-recommender', payload, result);
    res.json({ success: true, feature: 'process-change-recommender', ...result });
  } catch (err) {
    console.error('process-change-recommender analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Process-change recommendation engine reducing defect rates. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'process-change-recommender', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'process-change-recommender']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'process-change-recommender', title: 'Process-change recommendation engine reducing defect rates', project: 'AIVisualQAInspector' });
});
